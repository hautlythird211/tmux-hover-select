...
def get_screen_size():
    """Get screen resolution from wlr-randr, swaymsg, xrandr, or /sys/class/drm."""
    try:
...
def get_pane_geometry(screen_w, screen_h):
    """Query tmux for pane positions and sizes."""
    try:
        out = subprocess.check_output(
            ["tmux", "list-panes", "-F",
...
def main():
    ...
    pane_refresh_interval = 0.5

    def cleanup(*_):
        try:
            fcntl.ioctl(fd, 0x40044501, 0)
        except OSError:
            pass
        try:
            os.close(fd)
